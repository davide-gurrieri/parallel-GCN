...
public:
  ...
  std::vector<smart_stream> backward_streams; // size = n_layers

  smart_event start_backward;
  smart_event start_set_input;
  std::vector<smart_event> start_matmul_forward;  // size = n_layers
  std::vector<smart_event> start_matmul_backward; // size = n_layers - 1
  explicit GCNSmartObjects(const natural n_layers);
  GCNSmartObjects() = default;
};
...
class GCN
{
  ...
  void insert_layer(const natural input_dim, const natural output_dim, const real dropout, const natural layer_number);
