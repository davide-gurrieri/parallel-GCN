...
for (name in names) 
{
  ...
  x_axis_labels <- min(data$num_blocks_factor):max(data$num_blocks_factor)
  title = name
  
  plot = ggplot(data = data, aes(x=num_blocks_factor, y=avg_epoch_training_time, color = factor(num_threads))) +
  ...
  save_svg_plot(plot, name=name, folder = "../output/plot/", type = "ggplot")
}
